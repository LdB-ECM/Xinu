...
static thread lookupTest(unsigned char *request, int rqlen, unsigned char *reply,
                         int rplen)
{
    unsigned char buf[ELOOP_BUFSIZE];
...
thread test_arp(bool verbose)
{
	...
	{
		testSkip(TRUE, "no network interface");
		return OK;
	}
    ...
    data = (unsigned char *)(&_binary_data_testarp_pcap_start);
