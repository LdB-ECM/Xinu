...
void test_semWaiter(semaphore s, int times, unsigned char *testResult)
{
    while (times > 0)
    {
        wait(s);
...
bool test_checkProcState(tid_typ tid, unsigned char state, bool verbose)
{
    char msg[50];
...
bool test_checkResult(unsigned char testResult, unsigned char expected, bool verbose)
{
    char msg[80];
