...
static const struct {
    ...
    unsigned int nargs;
    unsigned int args[4]; /* Assumes all args are 4 bytes. */
} fprintf_specs[] = {
    { /* Empty string */
        .format = "",
        .expected_output = "",
    ...
    { /* Large min width */
        ...
        .expected_output = "     ""     ""     ""     ""123",
        .nargs = 1,
        .args = {(unsigned int)"123"},
    },
    { /* Large max width */
        .format = "%.23s",
        .expected_output = "abcdefghijklmnopqrstuvw",
        .nargs = 1,
        .args = {(unsigned int)"abcdefghijklmnopqrstuvwxyz"},
    },
    { /* Zero */
        .format = "%d",
        .expected_output = "0",
    ...
    { /* Combination of different conversion specifiers */
        ...
        .nargs = 4,
        .args = {4000, (unsigned int)"foobar", 0xfe, 'X'},
    },
};
...
static bool do_detailed_fprintf_tests(bool verbose, bool passed)
{
    ...
    for (i = 0; i < sizeof(fprintf_specs) / sizeof(fprintf_specs[0]); i++)
    {
        ...
        unsigned int nargs = fprintf_specs[i].nargs;
        const unsigned int *args = fprintf_specs[i].args;
        int ret;
        int len = strlen(expected_output);
        unsigned char obuf[len + 1];
...
struct fscanf_arg {
    unsigned long expected_result; /* Assumes a pointer can fit in here */
    enum {
        NONE_SCAN = 0,
        LONG_SCAN,
        INT_SCAN,
...
#define LONG_ARG(n) { (unsigned long)(n), LONG_SCAN }
#define INT_ARG(n) { (unsigned long)(int)(n), INT_SCAN }
#define SHORT_ARG(n) { (unsigned long)(short)(n), SHORT_SCAN }
#define STRING_ARG(n) { (unsigned long)(char*)(n), STRING_SCAN }
