...
static void t5(int times, unsigned char *testArray, int *index)
{
...
thread test_recursion(bool verbose)
{
    ...
    bool passed = TRUE;
    unsigned char testArray[4 * 2 * TIMES];
    unsigned char expectedResults[4 * 2 * TIMES];
    char str[50];
    int index = 0;
