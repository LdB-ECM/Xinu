 ...
 */
int tcpSend(struct tcb *tcbptr, unsigned char ctrl, unsigned int seqnum,
            unsigned int acknum, unsigned int datastart, unsigned short datalen)
{
    ...
    if (msslen)
    {
        ...
        *((unsigned short *)data) = hs2net((tcbptr->rcvmss + TCP_HDR_LEN));
        data += TCP_OPT_MSS_LEN;
