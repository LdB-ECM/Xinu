...
extern const struct usb_device_driver smsc9512_driver;
...
usb_status_t smsc9512_modify_reg (struct usb_device *udev, uint32_t index, uint32_t mask, uint32_t set);
usb_status_t smsc9512_set_reg_bits (struct usb_device *udev, uint32_t index, uint32_t set);
usb_status_t smsc9512_wait_device_attached (unsigned short minor);
...
xinu_devcall smsc9512Open(device *devptr, va_list ap);
xinu_devcall smsc9512Write(device *devptr, const void *buf, unsigned int len);

usb_status_t smsc9512_set_mac_address (struct usb_device *udev, const uint8_t *macaddr);
usb_status_t smsc9512_get_mac_address (struct usb_device *udev, uint8_t *macaddr);
usb_status_t smsc9512_set_loopback_mode(struct usb_device *udev, unsigned int on_off);
#endif /* _SMSC9512_H_ */
