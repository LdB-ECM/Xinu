 ...
 *      ::SYSERR.
 */
xinu_devcall udpWrite(device *devptr, const void *buf, unsigned int len)
{
    struct udp *udpptr;
    int result;
    ...
    else
    {
        ...
        {
            unsigned int bytes_remaining = len - count;
            ...
            result = udpSend(udpptr, pktsize, (const unsigned char*)buf + count);
            if (OK != result)
            {
                if (0 == count)
                {
