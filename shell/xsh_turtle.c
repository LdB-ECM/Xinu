...
unsigned long color; //current pen color
...
#define MAXY (fbtab[0].height_p - MINISHELLMINROW * fbtab[0].scrFontHt - 2)
#define STARTX (fbtab[0].width_p / 2)
...
char* colorfinder(unsigned long);
void move(int);
void turn(int);
void test(void);
void colorset(char*);
...
char* colorfinder(unsigned long color) {
    int i; 
    for (i = 0; i < ncolor; i++) {
...
void reset() {
    ...
    drawSegment(0, 0, fbtab[0].width_p-1, 0, RASPBERRY);
    drawSegment(0, 0, 0, MAXY, RASPBERRY);
    drawSegment(0, MAXY, fbtab[0].width_p -1, MAXY, RASPBERRY);
    drawSegment(fbtab[0].width_p -1, 0, fbtab[0].width_p -1, MAXY, RASPBERRY);
    //draw initial turtle.    
    drawTurtle(x, y, orientation);
