...
struct voipPkt
{
    unsigned int seq;
    unsigned int len;
    unsigned char buf[SEQ_BUF_SIZE];
};

thread serial_loop(unsigned short uart);
thread basic_send(unsigned short uart, unsigned short udp);
thread basic_receive(unsigned short uart, unsigned short udp);
thread seq_send(unsigned short uart, unsigned short udp);
thread seq_receive(unsigned short uart, unsigned short udp);
int toggle_sampling(unsigned short);
...
shellcmd xsh_voip(int nargs, char *args[])
{
    ...
    unsigned short uart = NULL, udpRx = NULL, udpTx = NULL;
    unsigned short mode = MODE_SERIAL, check = CHECK_BASIC;
...
thread seq_receive(unsigned short uart, unsigned short udp)
{
    unsigned int len, seq = 0;
    unsigned long ticks = 0, time = 0;
    struct voipPkt *voip;
    voip = malloc(sizeof(struct voipPkt));
...
int toggle_sampling(unsigned short uart)
{
    int i;
