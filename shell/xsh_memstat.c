...
{
    int i;
    unsigned int phys = 0;              /* total physical memory          */
    unsigned int resrv = 0;             /* total reserved system memory   */
    unsigned int code = 0;              /* total Xinu code memory         */
    unsigned int stack = 0;             /* total stack memory             */
    unsigned int kheap = 0;             /* total kernel heap memory       */
    unsigned int kused = 0;             /* total used kernel heap memory  */
    unsigned int kfree = 0;             /* total free memory              */
    struct memblock *block;				/* memory block pointer           */
#ifdef UHEAP_SIZE
    unsigned int uheap = 0;             /* total user heap memory         */
    unsigned int uused = 0;             /* total used user heap memory    */
...
#else
    phys = platform.maxaddr - platform.minaddr;
    resrv = (uintptr_t)&_start - platform.minaddr;
#endif
    
    /* Calculate amount of text memory */
    code = (uintptr_t)&_end - (uintptr_t)&_start;
    ...
    for (i = 0; i < NTHREAD; i++)
    {
        ...
        {
            stack += (unsigned long)thrtab[i].stklen;
        }
    }
...
static void printFreeList(struct memblock *base, char *ident)
{
    ...
    {
        printf("0x%08lX   %8u\n", (unsigned long)block, block->length);
    }
    printf("\n");
}
