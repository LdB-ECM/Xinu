...
static unsigned long string_to_addr(const char *s);

#define INVALID_ADDR (~(unsigned long)0)

static bool address_range_sane(unsigned long start, unsigned long len)
{
    if (len == 0)
    {
        return TRUE;
...
#else
    return ((unsigned long)platform.minaddr <= start &&
            start + len <= (unsigned long)platform.maxaddr);
#endif
}
...
shellcmd xsh_memdump(int nargs, char *args[])
{
    ...
    {
        hexdump((unsigned char *)start, length, canon);
    }
    else
    {
...
static unsigned long string_to_addr(const char *string)
{
    unsigned long n;
    ...
    else if (string[0] == '0' && isdigit((unsigned char)string[1]))
    {
        format = "%lo";
    }
