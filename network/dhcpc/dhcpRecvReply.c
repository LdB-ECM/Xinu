                               ...
                               struct packet *pkt)
{
    ...
    int found_msg;
    int retval;
    const unsigned char *gatewayptr;
    const unsigned char *maskptr;
    const unsigned char *opts_end;
        ...
        for (;;)
        {
            ...
            case DHCP_OPT_SERVER:
                if (len >= IPv4_ADDR_LEN)
                {
                    ...
                    serverIpv4Addr = ((unsigned int)opts[0] << 24) |
                                     ((unsigned int)opts[1] << 16) |
                                     ((unsigned int)opts[2] << 8) |
                                     ((unsigned int)opts[3] << 0);
                }
                break;
            }
