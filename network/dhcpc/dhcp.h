...
struct dhcpPkt
{
    unsigned char op;			/**< Operation (request or reply) */
    unsigned char htype;		/**< Hardware type (ethernet)     */
    unsigned char hlen;			/**< Hardware length              */
    unsigned char hops;			/**< Hops                         */

    unsigned int xid;			/**< Transfer Identification      */
    unsigned short secs;		/**< Seconds still start time     */
    ...
    unsigned int cookie;		/**< Magic cookie                 */
    unsigned char opts[1];		/**< Options                      */
};
...
xinu_syscall dhcpSendRequest (int descrp, struct dhcpData *data);
xinu_syscall dhcpRecvReply(int descrp, struct dhcpData *data, unsigned int timeout);
