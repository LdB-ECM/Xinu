...
uintptr_t memheap;              /* Bottom of heap (top of O/S stack)   */
unsigned long cpuid;            /* Processor id                        */

struct platform platform = { 0 };/* Platform specific configuration     */
...
void nulluser(void)
{
    ...
    ready(create(main, INITSTK, INITPRIO, "MAIN", 0));
	resched();
...
static int sysinit(void)
{
    ...
    memlist.length = (unsigned int)(platform.maxaddr - memheap);
    pmblock->next = NULL;
    pmblock->length = (unsigned int)(platform.maxaddr - memheap);
    ...
    thrptr->stklen = memheap - (uintptr_t)&_end;
    thrptr->stkptr = 0;
    ...
    /* Initialize user memory manager */
    {
        ...
        {
            userheap = (void *)((unsigned int)userheap - UHEAP_SIZE + sizeof(int));
    ...
    for (i = 0; i < NDEVS; i++)
    {
        if (devtab[i].init) devtab[i].init((device*)&devtab[i]);
    }
