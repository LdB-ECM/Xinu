...
struct telnet
{
    ...
    /* TELNET fields */
    unsigned char state;        /**< TELNET_STATE_* above               */
    unsigned char flags;        /**< Flags for Telnet options           */
    unsigned char echoState;    /**< State of echo option negotation    */
    ...
    unsigned int icount;        /**< Number of chars in input buffer    */
    unsigned int istart;        /**< Index of first char in "in" buffer */
    semaphore isem;             /**< Semaphore for input buffer         */

    /* TELNET output fields */
    char out[TELNET_OBLEN];     /**< Output buffer                      */
    unsigned int ocount;        /**< Number of characters in out buffer */
    unsigned int ostart;        /**< Index of first char in out buffer  */
...
xinu_devcall telnetInit(device *);
xinu_devcall telnetOpen(device *, va_list);
xinu_devcall telnetClose(device *);
xinu_devcall telnetRead(device *, void *, unsigned int);
xinu_devcall telnetWrite(device *, const void *, unsigned int);
xinu_devcall telnetGetc(device *);
xinu_devcall telnetPutc(device *, char);
xinu_devcall telnetControl(device *, int, long, long);
xinu_devcall telnetFlush(device *);
thread telnetServer(int, int, unsigned short, char *);
