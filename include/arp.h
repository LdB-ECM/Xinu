...
struct arpPkt
{
    ...
    unsigned char hwalen;               /**< Hardware address length         */
    unsigned char pralen;               /**< Protocol address length         */
    unsigned short op;                  /**< Operation                       */
    unsigned char addrs[1];             /**< Src & dst hdwr & proto addrs    */
};

/* ARP Table Entry */
struct arpEntry
{
    unsigned short state;               /**< ARP state for entry           */
    struct netif *nif;                  /**< Network interface             */
    struct netaddr hwaddr;              /**< Hardware address              */
    struct netaddr praddr;              /**< Protocol address              */
    unsigned int expires;               /**< clktime when entry expires    */
...
xinu_syscall arpFree(struct arpEntry *);
xinu_syscall arpInit(void);
xinu_syscall arpLookup(struct netif *, const struct netaddr *, struct netaddr *);
xinu_syscall arpNotify(struct arpEntry *, xinu_message);
xinu_syscall arpRecv(struct packet *);
xinu_syscall arpSendRqst(struct arpEntry *);
xinu_syscall arpSendReply(struct packet *);
